...
```{r plot aqiNyear, message=FALSE, warning=FALSE, fig.height=6}
g <- ggplot(data = aqicny) +
  theme_tufte() + theme(legend.position = "none") +
  geom_point(aes( x = time,y = pm2.5, col = year), size = 0.5) +
...
```{r exploreGDP, echo=TRUE, message=FALSE, warning=FALSE, fig.width=3, fig.align='center'}
g <- ggplot(data = gdpGrowth, aes( x = Year,y = provincial.gdp.Growth), group = city) +
  theme_tufte() + theme(legend.position = "none") +
  geom_segment( aes(x=Year, xend=Year, y=0, yend=provincial.gdp.Growth, group = city)) +
  geom_point( aes(col = city), size=1, fill=alpha("grey", 0.3), alpha=0.7, shape=21, stroke=1) +
