---
title: "About yield curve, summer 2019, part 1"
date: "2019-08-17T13:39:46+02:00"
author: "Datapleth.io"
tags: ["getting data", "treasury"]
categories: ["economy"]
...
short, it's about prediction of economic future. The objective of these two
posts is to replicate these visuals and to extend to other countries such as France.
...
We create a function to read the remote XML, parse it and convert it as a data.table.
...
And we convert horizon to numerical values in months, add a data columne as 
numeric.
...
We make a quick plot, it's always usefull to explore the data. Here we discover
that there is a strange point on 2017-04-14 for which all rates for all horizons
are equal to zero, we decide to remove these.
Rates on 2 month are only avaible recently we decide to remove these as well.
...
plot_ly(
    ...
    plotly::layout(
        ...
        scene=list(
            ...
            aspectmode = "manual",
            aspectratio = list(x=4,y=2,z=1.5),
            camera = list(
                eye = list(x = 1.9, y = -4, z = 1 ),
                center = list( x = 0.5, y = 0, z = 0)
            )
        )
    ) %>% 
...
f <- data.table::melt(
    c,
    measure.vars = c(
        "X1", "X3", "X6", "X12", "X24", "X36", "X60",
        "X84", "X120", "X240", "X360"),
    variable.name = "horizon",
    value.name = "ratio")
g <- ggplot(f)
g <- g + geom_point(aes(x = all_usa_data_long$NEW_DATE,
                        y = ratio,
                        col = horizon),
                    alpha = 0.5, size = 0.5)
g <- g + facet_grid(facets =  horizon ~ . )
